...
def store_embeddings_in_redis(redis_client,embeddings):
    for key, embedding in embeddings.items():
        redis_client.set(key, json.dumps(embedding))

def generate_and_store_embeddings_main(s3_client,aws_s3_bucket,s3_file_key,redis_client,openai_api_client):
    try:
        ...
        # import pdb;pdb.set_trace()
    try:
        embeddings = embed_chunks(openai_api_client,chunks)
    except Exception as e:
        print ("The function embed_chunks failed due to ", e)
        # import pdb;pdb.set_trace()
    try:
        store_embeddings_in_redis(redis_client,embeddings)
    except Exception as e:
